...
#pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) if (num_used_indices >= 1024)
...
void Metadata::PartitionLabel(const std::vector<data_size_t>& used_indices) {
  ...
  label_ = std::vector<label_t>(num_data_);
#pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) if (num_data_ >= 1024)
...
#pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS())
...
void Metadata::SetInitScore(const double* init_score, data_size_t len) {
  ...
  #pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) if (num_init_score_ >= 1024)
...
void Metadata::SetWeights(const label_t* weights, data_size_t len) {
  ...
  #pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) if (num_weights_ >= 1024)
...
void Metadata::SetQuery(const data_size_t* query, data_size_t len) {
  ...
  data_size_t sum = 0;
  #pragma omp parallel for schedule(static) num_threads(OMP_NUM_THREADS()) reduction(+:sum)
...
void Metadata::SetPosition(const data_size_t* positions, data_size_t len) {
  ...
  #pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) if (num_positions_ >= 1024)
  for (data_size_t i = 0; i < num_positions_; ++i) {
