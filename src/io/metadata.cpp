...
#pragma omp parallel for num_threads(1) schedule(static, 512) if (num_used_indices >= 1024)
...
void Metadata::PartitionLabel(const std::vector<data_size_t>& used_indices) {
  ...
  label_ = std::vector<label_t>(num_data_);
#pragma omp parallel for num_threads(1) schedule(static, 512) if (num_data_ >= 1024)
...
void Metadata::SetInitScore(const double* init_score, data_size_t len) {
  ...
  #pragma omp parallel for num_threads(1) schedule(static, 512) if (num_init_score_ >= 1024)
...
void Metadata::SetWeightsFromIterator(It first, It last) {
  ...
  #pragma omp parallel for num_threads(1) schedule(static, 512) if (num_weights_ >= 1024)
...
void Metadata::SetQuery(const data_size_t* query, data_size_t len) {
  ...
  data_size_t sum = 0;
  #pragma omp parallel for num_threads(1) schedule(static) reduction(+:sum)
...
void Metadata::SetPosition(const data_size_t* positions, data_size_t len) {
  ...
  #pragma omp parallel for num_threads(1) schedule(static, 512) if (num_positions_ >= 1024)
  for (data_size_t i = 0; i < num_positions_; ++i) {
