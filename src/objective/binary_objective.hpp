...
class BinaryLogloss: public ObjectiveFunction {
 public:
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    #pragma omp parallel for schedule(static) num_threads(OMP_NUM_THREADS()) reduction(+:cnt_positive, cnt_negative)
  ...
  double BoostFromScore(int) const override {
    ...
    double sumw = 0.0f;
    if (weights_ != nullptr) {
      #pragma omp parallel for schedule(static) num_threads(OMP_NUM_THREADS()) reduction(+:suml, sumw) if (!deterministic_)
      for (data_size_t i = 0; i < num_data_; ++i) {
      ...
      #pragma omp parallel for schedule(static) num_threads(OMP_NUM_THREADS()) reduction(+:suml) if (!deterministic_)
      for (data_size_t i = 0; i < num_data_; ++i) {
