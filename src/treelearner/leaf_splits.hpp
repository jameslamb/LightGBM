...
#pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) reduction(+:tmp_sum_gradients, tmp_sum_hessians) if (num_data_in_leaf_ >= 1024 && !deterministic_)
...
#pragma omp parallel for schedule(static, 512) num_threads(OMP_NUM_THREADS()) reduction(+:tmp_sum_gradients, tmp_sum_hessians, tmp_sum_gradients_and_hessians) if (num_data_in_leaf_ >= 1024 && !deterministic_)
    for (data_size_t i = 0; i < num_data_in_leaf_; ++i) {
