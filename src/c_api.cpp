...
int LGBM_DatasetPushRowsWithMetadata(DatasetHandle dataset,
  ...
  const int max_omp_threads = 1; // p_dataset->omp_max_threads() > 0 ? p_dataset->omp_max_threads() : OMP_NUM_THREADS();
...
int LGBM_DatasetPushRowsByCSRWithMetadata(DatasetHandle dataset,
  ...
  const int max_omp_threads = const int max_omp_threads = 1; // p_dataset->omp_max_threads() > 0 ? p_dataset->omp_max_threads() : OMP_NUM_THREADS();
...
int LGBM_DatasetCreateFromCSRFunc(void* get_row_funptr,
  ...
  #pragma omp parallel for num_threads(1) schedule(static) private(thread_buffer)
